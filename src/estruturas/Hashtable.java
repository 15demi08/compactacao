...
public class Hashtable<K, V> implements java.lang.Iterable<K> {
    ...
    /**
     * Create a new hash table using Cuckoo Hashing, with a given number of
     * hashes.
     *
    ...
    private void insert(HashEntry entry, int[] backtrack, int i, int pos)
        ...
        if (i >= MAX_TRIES) {
            ...
            if (table[pos] == null) {
                ...
                elementCount++;
                return;
            } else if (pos == 0 && ((HashEntry) table[pos]).key.equals(entry.key)) {
    ...
    /**
     * Returns a value value associated with a given key, or null, if the key is
     * not found.
     *
     ...
     *
     * @return The ratio between the number of elements stored and the amount of
     * memory used for it.
     */
